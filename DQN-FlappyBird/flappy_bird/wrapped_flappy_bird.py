...
class GameState:
    def __init__(self):
        ...
        self.playery = int((SCREENHEIGHT - PLAYER_HEIGHT) / 3)
        self.basex = 0
    ...
    def step(self, action, is_train=True):
        ...
        if action != 0 and action != 1:
            raise ValueError('动作输入错误！')

        # action == 0: do nothing
        # action == 1: flap the bird
        if action == 1:
        ...
        # check if crash here
        isCrash = checkCrash({'x': self.playerx, 'y': self.playery, 'index': self.playerIndex}, self.upperPipes,
                             self.lowerPipes)
        if isCrash:
            isOver = True
            reward = -1
