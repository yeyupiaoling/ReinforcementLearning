...
def train(args):
    ...
    while True:
        ...
        for value, reward, done in list(zip(values, rewards, dones))[::-1]:
            ...
            gae = gae + reward + args.gamma * next_value.detach().numpy() * (1 - done) - value.detach().numpy()
            next_value = value
            R.append(paddle.to_tensor(gae + value.numpy()))
        R = R[::-1]
        ...
        for i in range(args.num_epochs):
            ...
            for j in range(args.batch_size):
                ...
                actor_loss = paddle.to_tensor((ratio * advantages).numpy() + actor_loss.numpy(), dtype='float32')
